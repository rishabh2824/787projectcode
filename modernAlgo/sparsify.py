...
def matched_vertices_from_matching(matching: Iterable[Edge]) -> Set[Node]:
    """
    Return the set of matched vertices from a matching edge list.
    """
    matched: Set[Node] = set()
    for u, v in matching:
        matched.add(u)
...
def unmatched_vertices(
    left_nodes: Sequence[Node],
    right_nodes: Sequence[Node],
    matching: Iterable[Edge],
) -> Tuple[List[Node], List[Node]]:
    """
    Return unmatched vertices in U and V separately.
    """
    matched = matched_vertices_from_matching(matching)

    unmatched_U = [u for u in left_nodes if u not in matched]
    unmatched_V = [v for v in right_nodes if v not in matched]

    return unmatched_U, unmatched_V
...
if __name__ == "__main__":
    ...
    M = sparsify_partial_matching(U, V, E, seed=42)
    print("Sparsified matching M:", M)
    print("Size of M:", len(M))

    unmatched_U, unmatched_V = unmatched_vertices(U, V, M)
    print("Unmatched U:", unmatched_U)
    print("Unmatched V:", unmatched_V)
